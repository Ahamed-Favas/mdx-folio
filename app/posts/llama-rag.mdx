...
The first search result (search_results[0]) is chosen as the best match, and wikipedia.page(page_title, auto_suggest=False) is used to fetch the full article, avoiding automatic redirections that might select an incorrect page.
...
```
## What is LangChain?
Now we have fresh data from Wikipedia, but to need prepare it before use with RAG, so that we need some set of tools, LangChain is a open-source framework which provides a set of tools and components that make it easier to build complex AI applications, including features for document loading, text splitting, and integrating with various AI models and databases.
...
<br/>
Then in your terminal use these commands to download a language model and an embedding model, Here I chose Llama llama3.2:1b as model, and all-minilm as embedding model:
