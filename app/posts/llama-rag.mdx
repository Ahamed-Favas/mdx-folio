...
In this post, we’ll walk through a Python code that demonstrates how to build a question-answering system using Wikipedia, text splitting, vector embeddings, and a language model (Llama). The system retrieves relevant context from a Wikipedia article and uses that context to answer a user’s question.


# What is RAG (Retrieval-Augmented Generation), and why?

Suppose you have a large language model (LLM) running on your machine (we will install one later in this post) and if you ask it a simple question, such as "Who is the current President of the United States?" The model might respond:
...
In this step, we’re defining our question and 
wikipedia returns a list of potential article titles related to the query.
This helps find the most relevant Wikipedia page without directly specifying an article.

The first search result (search_results[0]) is chosen as the best match.
wikipedia.page(page_title, auto_suggest=False) is used to fetch the full article, avoiding automatic redirections that might pick incorrect pages.

also we added error messages if there is no page found, In such cases try to make your query more general.
...
SO that we are going to split page got from Wikipedia
<br/>
...
```
## What is LangChain?
Now we have fresh data from Wikipedia, but to make that data to use with RAG, we need some set of tools, LangChain is a open-source framework which provides a set of tools and components that make it easier to build complex AI applications, including features for document loading, text splitting, and integrating with various AI models and databases.


also we need to install text-splitters from LangChain
...
Important: Download Ollama on your machine from their <a href="https://ollama.com/download" target="_blank" rel="noopener noreferrer" style={{'color': '#87CEEB'}}>official website</a>
...
<br/>
Then in your terminal use these commands to download a language model and an embedding model, Here I chose Llama llama3.2:1b as model, and all-minilm as embedding model, also note that the more 
...
```
This system allows us to find the most relevant documents based on our question "Who is the current president of America?", using similarity search to retrieve the top 1 match.
...
for that we need python api for ollam
...
While Testing the results from llm, sometimes I got false informations, even though provided context is reliable
So it mainly depends on the capabilities of llm.

The Notebook is provided on https://github.com/Ahamed-Favas/Blog-RAG